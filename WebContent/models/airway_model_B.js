...
Airway.prototype.mc_response = function(max_asm, areas, logd)
{
    ...
    if(Math.PI*Math.pow(radii.asm, 2) > areas.asm + areas.sub_mucosal + areas.mucosal) {
        // Normal: work outside in
        //radii.asm = max_asm * (1 - this.shortening(logd));
        ...
        // Error: work inside out
        //console.log("error");
        radii.lumen = 0;
        radii.mucosal = Math.sqrt((areas.mucosal)/Math.PI);
        radii.sub_mucosal = Math.sqrt((areas.sub_mucosal+areas.mucosal)/Math.PI);
        radii.asm = Math.sqrt((areas.asm+areas.sub_mucosal+areas.mucosal)/Math.PI);
    }
